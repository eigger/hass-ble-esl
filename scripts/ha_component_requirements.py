...
requirements of individual components; the bluetooth stack (and its usb
dependency) and the recorder are needed to set this integration up. Reading
them from the installed HA keeps the pins in step with whatever HA version the
test package brings.

    pip install -r <(python scripts/ha_component_requirements.py bluetooth usb recorder)
"""
