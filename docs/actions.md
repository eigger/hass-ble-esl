...
Protocol-specific extras: PickSmart adds `start_probes` (how many START commands were needed — should mostly be 1), `sends` / `resends` (chunks the tag asked for again), `round_trip_ms` (per-chunk round trip; the best measure of path quality) and `completed_by_tag`; XTE and WOLINK add `chunk_size` (the ATT write size the backend allowed — for XTE: 20 on some proxies, 244 on others; for WOLINK: 200 to 506 based on MTU; which dominates `transfer_s`).

Reading it (the [troubleshooting guide](troubleshooting.md) goes case by case): start with `failed_stage`. An `error` beginning *The link dropped* means the link went away mid-session — the wait ends there rather than running the step's timeout out, so `transfer_s` / `finish_s` show when the link went, not a full timeout. `unreachable` / `connect` with a low `rssi` or `paths: 1` points at placement or a missing proxy; `auth` (`failed_detail: handshake`) is the tag not answering after the link is up (PickSmart `start_probes: 3`, a WOLINK auth error); `finish` is the panel, not the link. Within a `transfer` failure, `resends` or `start_probes` above 1 point at a marginal link (a retry after a failure *during* the transfer is automatically paced slower — `pacing_s` — while a retry after a connect or `auth` failure runs at full speed; if the counters stay high, move the tag or proxy); a large `finish_s` on WOLINK/easyTag is the panel refresh, which grows with panel size and cold temperature and is not a transport problem. `via` tells you which proxy the tag actually used, which is what to move or replace.
