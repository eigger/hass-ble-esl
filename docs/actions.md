...
Protocol-specific extras: PickSmart adds `start_probes` (how many START commands were needed — should mostly be 1), `sends` / `resends` (chunks the tag asked for again), `round_trip_ms` (per-chunk round trip; the best measure of path quality) and `completed_by_tag`; XTE and WOLINK add `chunk_size` (the per-chunk payload size: 20–244 on XTE, 200–506 on WOLINK based on MTU; larger sizes reduce transfer round trips and dominate `transfer_s`).
